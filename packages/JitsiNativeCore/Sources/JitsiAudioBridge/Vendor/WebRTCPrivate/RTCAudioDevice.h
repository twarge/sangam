// Vendored from WebRTC's Objective-C SDK, sdk/objc/components/audio/RTCAudioDevice.h,
// pinned to the M124 build this package links against.
//
// The distributed WebRTC.framework declares `RTCPeerConnectionFactory`'s
// `initWithEncoderFactory:decoderFactory:audioDevice:` but ships no definition
// of the protocol that initializer takes — 84 headers, none of them this one.
// Implementing a custom audio device therefore needs the declaration carried
// here, exactly as `SGAudioTrackTap` carries the C++ headers it reaches
// through. Its selector set was checked against the shipped binary before
// vendoring; never update the binary independently of this file.
//
// Note the contract in the protocol comment below: an implementation is
// "fully responsible for configuring application's AVAudioSession". Adopting
// this replaces CallKitAudioBridge's manual-audio handshake rather than
// adding to it.
